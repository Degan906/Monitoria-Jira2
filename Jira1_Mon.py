...
card_tooltips = {
   ...
   "🔽 Opera != Opera": 'Busca por todas as issues do tipo "Supply Chain" criadas a partir de 1º de maio de 2025 no projeto SUPPLY, pertencentes à empresa "Opera Jarinu (HG)", "Carbon Jarinu" e Vertico e não estão associadas ao nível "Compras HG" ou "Carbon Vertco"',
   "🔽 RNC != Vidro": 'Busca todas as issues do projeto RNC que estão no status "Em Produção Carbon" e não estão vinculadas com o tipo de link "RNC > Vidro"',
   "⏫ TI - Opera": 'Essa JQL busca todas as issues do projeto "TI" onde o campo empresa (do tipo dropdown) está com os valores "High Glass" ou "Opera Jarinu (HG)", o campo level não está com os valores "HG" ou "VERTCO", e a issue ainda não foi resolvida (campo resolution = Unresolved).',
   "⏫ Compras Parados": 'Pedidos de compras que nao vao para o fluxo de aprovação Diretoria e Comite não deve ficar dessas etapas - resolution = Unresolved AND project = COM AND status IN ("Aguardando aprovação Diretoria", "Aguardando Comite") and "Request Type" in ("Vidros (COM)", "RNC (COM)", "Assistência Técnica-Vidros (COM)") and "Empresa[Dropdown]" = "Carbon Assistência"',
...
card_links = {
   ...
   "⏫ Compras Supply":"https://carboncars.atlassian.net/issues/?jql=project%20%3D%20COM%20and%20issuelinktype%20is%20empty%20and%20created%20%3E%3D%20%222023-07-01%22%20and%20status%20NOT%20IN%20%28%22Aguardando%20aprova%C3%A7%C3%A3o%20Diretoria%22%2C%20%22Aguardando%20Comite%22%29%20and%20resolution%20%3D%20Unresolved",
   ...
   "⏫ Vidros Label":"https://carboncars.atlassian.net/issues/?jql=project%20%3D%20AP%20and%20JSW_P-Vidro%20~%20Done%20and%20labels%20IN%20%28V%29%20AND%20status%20%21%3D%20Cancelado",
   ...
   "⏫ Compras Parados":"https://carboncars.atlassian.net/issues/?jql=resolution%20%3D%20Unresolved%20AND%20project%20%3D%20COM%20AND%20status%20IN%20%28%22Aguardando%20aprova%C3%A7%C3%A3o%20Diretoria%22%2C%20%22Aguardando%20Comite%22%29%20and%20%22Request%20Type%22%20in%20%28%22Vidros%20%28COM%29%22%2C%20%22RNC%20%28COM%29%22%2C%20%22Assist%C3%AAncia%20T%C3%A9cnica-Vidros%20%28COM%29%22%2C%20%22MP%20e%20Insumos%20-%20Opera%22%29%20or%20resolution%20%3D%20Unresolved%20AND%20project%20%3D%20COM%20AND%20status%20IN%20%28%22Aguardando%20aprova%C3%A7%C3%A3o%20Diretoria%22%2C%20%22Aguardando%20Comite%22%29%20%20%20and%20%22Empresa%5BDropdown%5D%22%20%3D%20%22Carbon%20Assist%C3%AAncia%22",
   "⏫ Laudos Concluídos PB":"https://carboncars.atlassian.net/issues/?jql=project%20%3D%20PB%20and%20status%20changed%20from%20%2212%20-%20Aguardando%20Valida%C3%A7%C3%B5es%22%20to%20%22132.1%20-%20Laudo%20Conclu%C3%ADdo%22%20and%20status%20%3D%20%22132.1%20-%20Laudo%20Conclu%C3%ADdo%22",
   "⏫ PB Finalizados":"https://carboncars.atlassian.net/issues/?jql=project%20%3D%20PB%20and%20status%20%3D%20%22114.2%20-%20Finalizado%20Prepara%C3%A7%C3%A3o%20Entrega%22%20AND%20%22JSW_P-Valida%C3%A7%C3%A3o%20%28TM%29%20-%20Done%22%20is%20NOT%20EMPTY",
    ...
    if menu_option == "Dash de monitoria":
        ...
        queries = {
            "🤖 AUTOMAÇÕES AP 🤖": {
                ...
                "⏫ Compras Supply": 'project = COM and issuelinktype is empty and created >= "2023-07-01" and status NOT IN ("Aguardando aprovação Diretoria", "Aguardando Comite") and resolution = Unresolved',
                ...
                "⏫ Tensylon Label": 'project = AP and "JSW_P-Tensylon[Short text]" ~ Done and labels IN (T) AND status != Cancelado',
                "⏫ Vidros Label": 'project = AP and JSW_P-Vidro ~ Done and labels IN (V) AND status != Cancelado',
                ...
                "⏫ Compras Parados": 'project = COM AND status IN ("Aguardando aprovação Diretoria", "Aguardando Comite") AND "diretor responsável[user picker (single user)]" = empty or resolution = Unresolved AND project = COM AND status IN ("Aguardando aprovação Diretoria", "Aguardando Comite") and "Request Type" in ("Vidros (COM)", "RNC (COM)", "Assistência Técnica-Vidros (COM)", "MP e Insumos - Opera") or project = COM AND resolution = Unresolved AND project = COM AND status IN ("Aguardando aprovação Diretoria", "Aguardando Comite")   and "Empresa[Dropdown]" = "Carbon Assistência"',
